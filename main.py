...
<body class="bg-slate-950 text-gray-200 font-sans h-screen flex flex-col">
    <header class="bg-slate-900 border-b border-slate-800 p-4 shadow-lg flex justify-between items-center z-10 shrink-0 relative">
        <div class="flex items-center gap-6">
            <h1 class="text-xl font-bold text-indigo-400">StoryStash <span class="text-xs text-gray-500">v3</span></h1>
    ...
    <main class="flex-1 overflow-hidden w-full flex justify-center p-6 gap-6 relative" style="{{ 'background-image: url(/backgrounds/' + background_file + '); background-size: cover; background-position: center;' if background_file else '' }}">
        ...
        {% if mode == 'dashboard' %}
            <div class="flex-1 max-w-6xl h-full overflow-y-auto space-y-8 z-10">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="bg-slate-900 border border-slate-800 p-6 rounded-xl shadow-lg flex items-center gap-4"><div class="w-12 h-12 rounded-full bg-indigo-900/50 text-indigo-400 flex items-center justify-center text-xl"><i class="fas fa-book"></i></div><div><div class="text-2xl font-bold text-white">{{ stats.total_stories }}</div><div class="text-xs uppercase text-gray-500 font-bold tracking-wider">Total Stories</div></div></div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div class="lg:col-span-2">
                        ...
                        <div class="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
                            ...
                            {% endfor %}
                            {% if not recent_stories %}<div class="p-6 text-center text-gray-500">No stories found.</div>{% endif %}
                        </div>
                    </div>
                    <div>
        ...
        {% elif mode == 'search' %}
            <div class="flex-1 max-w-4xl h-full overflow-y-auto z-10">
                ...
                <div class="space-y-4">
                    ...
                    <div class="bg-slate-900 border border-slate-800 rounded-xl p-6 hover:border-indigo-500 transition">
                        <a href="/read/{{ result.path }}" class="block">
                            <h3 class="font-bold text-lg text-indigo-400 mb-1"><i class="fas fa-file-alt mr-2"></i> {{ result.title }}</h3>
                            <div class="text-xs text-gray-500 mb-3">{{ result.path }}</div>
                            <div class="space-y-2">{% for match in result.matches %}<div class="text-sm text-gray-300 font-mono bg-slate-950 p-2 rounded border-l-2 border-indigo-500/50">...{{ match }}...</div>{% endfor %}</div>
                        </a>
                    </div>
                    {% endfor %}
                    {% if not results %}<div class="text-center text-gray-500 mt-20"><i class="fas fa-ghost text-4xl mb-4 opacity-50"></i><p>No results found.</p></div>{% endif %}
                </div>
            </div>
        ...
        {% elif mode == 'stories_list' %}
            <aside class="w-64 shrink-0 bg-slate-900/50 rounded-xl border border-slate-800 p-4 h-full overflow-y-auto z-10">
                ...
                <nav class="space-y-1">
                    <a href="/stories" class="block px-3 py-2 rounded text-sm {{ 'bg-indigo-900/50 text-indigo-200' if not active_campaign else 'text-gray-400 hover:bg-slate-800 hover:text-white' }}"><i class="fas fa-layer-group w-5"></i> All Stories</a>
                    {% for camp in campaigns %}{% if camp != "Unsorted" %}<a href="/stories?campaign={{ camp }}" class="block px-3 py-2 rounded text-sm {{ 'bg-indigo-900/50 text-indigo-200' if active_campaign == camp else 'text-gray-400 hover:bg-slate-800 hover:text-white' }}"><i class="fas fa-folder w-5 text-yellow-600"></i> {{ camp }}</a>{% endif %}{% endfor %}
                </nav>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    ...
                    <div class="bg-slate-900 rounded-xl border border-slate-800 shadow-lg hover:border-indigo-500/50 transition flex flex-col h-[280px] group relative">
                        ...
                        <a href="/read/{{ item.path }}" class="flex-1 flex flex-col p-5">
                            <div class="mb-3"><h3 class="font-bold text-lg text-gray-100 leading-tight line-clamp-1" title="{{ item.meta.display_title }}">{{ item.meta.display_title }}</h3><div class="text-[10px] text-gray-500 mt-1 flex gap-2"><span><i class="far fa-clock"></i> {{ item.stats.date }}</span><span><i class="far fa-comment-alt"></i> {{ item.stats.msg_count }}</span></div></div>
                            <div class="flex-1 text-xs text-gray-400 italic line-clamp-4 leading-relaxed overflow-hidden">{{ item.meta.synopsis }}</div>
                            {% if item.stats.top_characters %}<div class="mt-3 flex -space-x-2 overflow-hidden py-1">{% for char in item.stats.top_characters %}<div class="inline-block h-6 w-6 rounded-full ring-2 ring-slate-900 bg-indigo-500 flex items-center justify-center text-[8px] font-bold text-white">{{ char[:1] }}</div>{% endfor %}</div>{% endif %}
                        </a>
                        <div class="p-3 border-t border-slate-800 bg-slate-950/30 rounded-b-xl flex gap-2 overflow-x-auto">{% if item.meta.tags %}{% for tag in item.meta.tags %}<span class="tag tag-blue">{{ tag }}</span>{% endfor %}{% else %}<span class="tag tag-gray">No Tags</span>{% endif %}</div>
                    </div>
                    {% endfor %}
                </div>
            </div>
        ...
        {% elif mode == 'read' %}
            ...
            <div class="flex-1 max-w-4xl h-full overflow-y-auto pr-2 z-10">
                <div class="glass-panel rounded-xl shadow-2xl p-8 min-h-full">
                    <div class="space-y-6">
                    {% for block in blocks %}
                        {% if block.type == 'ooc' %}
                        ...
                        {% elif block.type == 'dialogue' %}
                            <div class="flex flex-col space-y-1">
                                <span class="text-xs font-bold text-indigo-400 ml-1 drop-shadow-md">{{ block.speaker }}</span>
                                {% set speaker_char = char_map.get(block.speaker) %}
                                {% set bubble_color = speaker_char.bubble_color if speaker_char else '#1f2937' %}
                                <div class="text-gray-100 p-3 rounded-2xl rounded-tl-none inline-block max-w-[85%] self-start shadow-sm leading-relaxed border" style="background-color: {{ bubble_color }}DD; border-color: {{ bubble_color }};">
                        ...
                        {% else %}
                            <div class="text-gray-300 leading-relaxed bg-black/40 p-4 rounded-lg border-l-2 border-indigo-500/50 backdrop-blur-sm">{{ block.text | safe }}</div>
                        {% endif %}
                    {% endfor %}
                    </div>
    ...
    <dialog id="editModal" class="rounded-xl shadow-2xl bg-slate-900 border border-slate-700 text-gray-200 w-[500px] backdrop:bg-black/80 max-h-[90vh]">
        <form action="/update_character_details" method="post" enctype="multipart/form-data" class="flex flex-col h-full">
            <div class="p-6 border-b border-slate-800 bg-slate-900 sticky top-0 z-10"><h2 class="text-lg font-bold text-indigo-400">Edit <span id="modalCharNameDisplay"></span></h2><input type="hidden" id="modalCharIdInput" name="char_id" value=""><input type="hidden" name="return_file" value="{{ filename | default('') }}"></div>
            <div class="p-6 overflow-y-auto space-y-4">
                <div class="flex gap-4">
                    <div class="flex-1"><label class="text-xs text-gray-500 uppercase font-bold">Display Name</label><input type="text" id="modalNameInput" name="name" class="input-dark mt-1"></div>
                    <div><label class="text-xs text-gray-500 uppercase font-bold">Chat Color</label><div class="flex items-center gap-2 mt-1"><input type="color" id="modalBubbleColor" name="bubble_color" class="h-9 w-9 bg-transparent cursor-pointer rounded"></div></div>
                </div>
...
async def dashboard(request: Request):
    ...
    stats = {"total_stories": total_stories, "total_chars": len(all_chars), "total_campaigns": len(campaigns) - 1}
...
async def char_profile(char_id: str):
    ...
    return jinja_template.render(mode="char_profile", char=char, char_id=char_id, stories=linked_stories)
